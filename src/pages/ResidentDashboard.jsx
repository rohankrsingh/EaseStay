...
export default function ResidentDashboard({ session }) {
  ...
  const handleLeaveCommunity = async () => {
    if (!window.confirm('Are you sure you want to leave this PG? You will lose access to all your issues.')) return;
    try {
      const { error } = await supabase.from('members').delete().eq('user_id', session.user.id);
        ...
        <main className="flex-1 p-5 sm:p-8 md:p-12 overflow-y-auto w-full max-w-5xl mx-auto">
          ...
          {(activeTab === 'dashboard' || activeTab === 'overview') && memberInfo && memberInfo.status !== 'pending' && (
            <div className="space-y-10 w-full">
              ...
              {activeTab === 'dashboard' && (
                <>
                  <div className="space-y-4 bg-white p-6 sm:p-8 rounded-[2rem] border border-slate-200 shadow-sm">
